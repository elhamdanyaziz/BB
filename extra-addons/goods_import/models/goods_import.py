    ...
    def action_invoice_create(self, cr, uid, ids, journal_id, group=False, type='out_invoice', context=None):
                        ...
                        if move.product_id.id not in products :
                            ...
                            for mv in todo[key] :
                                if mv.product_id.id == move.product_id.id :
                                    cr.execute('update stock_move set product_uom_qty=%s,product_uos_qty=%s where id = %s',(mv.product_uom_qty+move.product_uom_qty, mv.product_uos_qty+move.product_uos_qty,mv.id,))
        ...
        for moves in todo.values():
            if picking.import_id :
                moves['import_id'] = picking.import_id.id
...
class product_template(models.Model):
        ...
        if self.type == 'product' :
            ...
            for section in sections :
                if section.location_id :
                    if section.location_id.id != 12 :
                        stock_historys = self.env['stock.history'].search([('location_id','=',section.location_id.id),('product_id','=',self.id)])
                        sum_qty_reservation += sum(histo.quantity for histo in stock_historys)
        self.total_reservation_qty = sum_qty_reservation
    ...
    total_reservation_qty = fields.Float('Résa BU',compute='_compute_qty_reservation', readonly=True, store=True)
