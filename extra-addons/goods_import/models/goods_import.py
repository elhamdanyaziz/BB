...
class goods_import(models.Model):
    ...
    amount_total = fields.Float('Total du dossier', compute='_compute_amount', readonly=True,store=True)
...
class purchase_order_line(models.Model):
    ...
    def onchange_product_id(self, cr, uid, ids, pricelist_id, product_id, qty, uom_id,
        ...
        name = ''
        if lang == 'en_US' :
            name = product.categ_id.english_description
        if lang == 'fr_FR' :
            name = product.categ_id.description
        res['value'].update({'name': name})
        ...
        for product in self:
            if product.type == 'product' :
                ...
                for section in sections :
                    if section.location_id and section.location_id.id != 12 and section.location_id.id != 9 :
                        ...
                        # stock_moves = self.env['stock.move'].search([('location_dest_id', '=', section.location_id.id), ('product_id', '=', product.id), ('state', '=', 'done')])
                        # sum_qty_reservation += sum(move.product_uom_qty for move in stock_moves)
