...
class account_voucher(models.Model):
    ...
    customer_bank = fields.Char(string='Banque du Client')
    city_customer_bank = fields.Char(string='Ville de Banque')
    ...
    def create(self,vals):
        ...
        if journal.is_jboe:
            ...
            account_boe = self.env['account.account'].search([('code', '=', account_code)])
            if account_boe:
                account_effect_id = account_boe.id
                vals['account_id'] = account_effect_id
    ...
    def proforma_voucher(self):
        ...
        if voucher.journal_id.is_species :
            ...
            for line in voucher.line_ids :
                ...
                account_timbre = self.env['account.account'].search([('code', '=', 4458000000)])
            ...
            if voucher.journal_id.is_bank_transfer :
                voucher.write({'state': 'encaissed'})
                if voucher.move_ids :
    ...
    def account_move_get(self, cr, uid, voucher_id, context=None):
        ...
        move = {
            'name': name,
            'journal_id': voucher.journal_id.journal_id.id if voucher.journal_id.journal_id else voucher.journal_id.id,
            'narration': voucher.narration,
            'date': voucher.date,
            'ref': ref,
            'period_id': voucher.period_id.id,
    ...
    def recompute_voucher_lines(self, cr, uid, ids, partner_id, journal_id, price, currency_id, ttype, date, context=None):
        ...
        for line in account_move_lines:
            ...
            if line.currency_id and currency_id == line.currency_id.id:
                ...
                # if line.invoice :
                #     if line.invoice.inv_species :
                #         amount_original = line.invoice.amount_total
                #         amount_unreconciled = line.invoice.amount_total
    ...
    def cancel_voucher(self, cr, uid, ids, context=None):
        ...
        for voucher in self.browse(cr, uid, ids, context=context):
            ...
            move_ids = move_pool.search(cr,uid,[('voucher_id','=',voucher.id)])
            voucher.refresh()
            for line in voucher.move_ids:
