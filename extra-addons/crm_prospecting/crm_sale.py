...
class sale_order(models.Model):
    ...
    all_planned_deliveries = fields.Boolean('BC planifié ?', compute='_compute_all_planned_deliveries')
    ...
    # @api.depends('order_line.product_uom_qty','order_line.qty_available')
    @api.depends('situation_product_ids.qty_to_program')
    def _compute_all_planned_deliveries(self):
        tab_all_planned_deliveries = []
        for order in self:
            for situation in order.situation_product_ids:
                if situation.qty_to_program == 0:
                    tab_all_planned_deliveries.append(True)
                # for line in order.order_line :
                #     qty = line.product_uom_qty - line.qty_available
                #     if qty == 0 :
                #         tab_all_planned_deliveries.append(True)
            if len(tab_all_planned_deliveries) == len(order.situation_product_ids) :
                order.all_planned_deliveries = True
            else :
                order.all_planned_deliveries = False
    ...
    def _compute_percentage_margin(self):
        for order in self :
            ...
            percentage_margin = (order.margin/order.amount_untaxed) * 100 if order.amount_untaxed != 0 else 0
    ...
    def _compute_reel_percentage_margin(self):
        for order in self :
            ...
            percentage_margin = (order.reel_margin/order.amount_untaxed)*100 if order.amount_untaxed != 0 else 0
    ...
    def action_create_removal_order(self):
        ...
        if self.section_id == 'CPT':
            raise except_orm(_('Attention!'),_('Ce client est un client comptoir,vous pouvez pas générer des bons d\'enlevement avec reservation.'))
...
class sale_order_line(models.Model):
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        # print  cr, uid, ids, pricelist, product, qty,uom, qty_uos, uos, name, partner_id,lang, update_tax, date_order, packaging, fiscal_position, flag, context
        warning = False
        ...
        result['product_available_qty'] = product_obj.virtual_available
...
class sale_order_situation(models.Model):
    ...
    def _compute_qty(self):
        for situation in self:
            # order_line = self.env['sale.order.line'].search([('order_id', '=', situation.sale_order_id.id), ('product_id', '=', situation.product_id.id)])
            # removal_lines = self.env['stock.move'].search([('sale_order_id', '=', situation.sale_order_id.id), ('product_id', '=', situation.product_id.id)])
            # print "order_line,removal_lines",order_line,removal_lines
            # if order_line:
            #     qty_programmed = order_line.qty_available
            qty_shipped = 0
            qty_remaining_deliver = 0
            qty_programmed = 0

            removals = self.env['stock.removal'].search([('sale_order_id', '=', situation.sale_order_id.id)])
            ...
            if removals:
                for removal in removals:
                    ...
                    removal_line = self.env['stock.move'].search([('sale_order_id', '=', situation.sale_order_id.id),('product_id', '=', situation.product_id.id),('removal_id', '=', removal.id)])
                    if removal_line:
                        if removal.state == 'ready_to_deliver':
                            qty_remaining_deliver += removal_line.product_uom_qty
                        if removal.state == 'done':
                            qty_shipped += removal_line.product_uom_qty
                        if removal.state in ('draft','validated','approuved'):
                            qty_programmed += removal_line.product_uom_qty
            situation.qty_programmed = qty_programmed
            situation.qty_remaining_deliver = qty_remaining_deliver
            situation.qty_shipped = qty_shipped
            situation.qty_to_program = (situation.qty_ordered-(situation.qty_programmed+situation.qty_canceled+situation.qty_remaining_deliver+situation.qty_shipped))
            # situation.qty_to_program = (situation.qty_ordered - (situation.qty_programmed+situation.qty_remaining_deliver+situation.qty_shipped))
            print "self.qty_programmed,self.qty_to_program,self.qty_remaining_deliver,self.qty_shipped",situation.qty_programmed,situation.qty_to_program,situation.qty_remaining_deliver,situation.qty_shipped

    product_id = fields.Many2one('product.product', string='Article')
    qty_ordered = fields.Float('Quantité Commandée', default=0)
    qty_canceled = fields.Float('Quantité Annulée', default=0)
    qty_shipped = fields.Float('Quantité Livrée', compute='_compute_qty')
    qty_remaining_deliver = fields.Float('Quantité à livrer', compute='_compute_qty')
    qty_to_program = fields.Float('Quantité à programmer', compute='_compute_qty')
    qty_programmed = fields.Float('Quantité programmée', compute='_compute_qty')
...
class stock_removal(models.Model):
    ...
    # reservation_method = fields.Selection([('stock', 'Réservation sur Stock'), ('shipping', 'Reservation sur Arrivage'), ('purchase', 'Reservation sur Demande Achat'),('none', 'Sans Réservation'),], 'Méthode de Réservation', required=False)
    reservation_method = fields.Selection([('stock', 'Réservation sur Stock'), ('shipping', 'Reservation sur Arrivage'), ('none', 'Sans Réservation'),], 'Méthode de Réservation', required=False)
        ...
        if values.get('sale_order_id') :
            ...
            if values.get('move_lines') :
                for move in values.get('move_lines') :
                    moves = self.env['stock.move'].search([('removal_id', 'in', removals._ids),('sale_order_line_id','=',move[2]['sale_order_line_id']),('state', '!=', 'cancel')])
                    ...
                    if sum_qty + move[2]['product_uom_qty'] > sale_order_line.product_uom_qty:
                        raise except_orm('Attention', 'La quantité à planifier est supérieure à la quantité vendue %s' % (sale_order_line.product_uom_qty))
                    else :
    ...
    @api.multi
    def action_cancel_removal_order(self):
        lines = []
        opp_id = False
        if self.opp_id :
            opp_id = self.opp_id.id
        for line in self.move_lines :
            line = (0,0, {'move_id':line.id,'opp_id':opp_id,'removal_id':self.id,'product_id' : line.product_id.id,'name' : line.product_id.name,'product_qty' :line.product_uom_qty,'product_uom_id':line.product_uom.id})
            lines.append(line)
        return {
            'name': 'Annulation BE',
            'view_type': 'form',
            'view_mode': 'form',
            'view_id': self.env['ir.ui.view'].search([('name', '=', 'stock.removal.cancel.form')])[0].id,
            'res_model': 'stock.removal.cancel',
            'type': 'ir.actions.act_window',
            'target': 'new',
            'context': {
                'default_filter':self.state,
    ...
    def action_cancel(self):
        ...
        # for move in self.move_lines :
        #     move.action_cancel()
        #reste a regler le pb annulation
        self.state = 'cancel'
        return True
    ...
    def action_create_picking_out(self):
        ...
        if self.whith_reservation:
            for move in self.move_lines:
                ...
                movelines.append((0, 0, picking_move))
...
class stock_normal_reservation(models.Model):
    ...
    def action_cancel(self):
        self.state = 'cancel'
        if self.removal_id.state == 'draft' or self.removal_id.state == 'validated':
            for reservation in self.reservation_lines :
                reservation.move_id.action_cancel()
        for reservation in self.reservation_lines:
...
class stock_shipping_reservation(models.Model):
    ...
    def action_cancel(self):
        ...
        if self.removal_id.state == 'draft' or self.removal_id.state == 'validated':
            for reservation in self.reservation_lines :
                if reservation.move_id :
                    reservation.move_id.action_cancel()
                if reservation.move_dest_id:
                    reservation.move_dest_id.action_cancel()
        for reservation in self.reservation_lines:
            reservation.write({'state': 'cancel'})
        return True
...
class stock_picking(models.Model):
    ...
    def action_cancel(self):
        ...
        if not self.order_id.whith_reservation:
            self.removal_id.action_cancel()
        return res
