...
class res_partner(models.Model):
    ...
    garanty = fields.Selection([('cs', 'Caution Solidaire'), ('cb', 'Caution Bancaire'),('cqs', 'Chèque Société'), ('cqp', 'Chèque Personnel')], 'Garantie')
    ...
    local_ok = fields.Boolean('Fournisseur Local ?', default=False)
    ...
    payment_earliest_due_date_related = fields.Date(related='payment_earliest_due_date', store=True, readonly=True, copy=False)
    payment_amount_due_related = fields.Float(related='payment_amount_due', store=True, readonly=True, copy=False)
    ice_code = fields.Char("ICE")
    name2 = fields.Char("Nom de Codification")
    ...
    def _payment_earliest_date_search(self, cr, uid, obj, name, args, context=None):
        ...
        cr.execute('SELECT partner_id FROM account_move_line l '\
                    'WHERE account_id IN '\
                        '(SELECT id FROM account_account '\
                        'WHERE type=\'receivable\' AND active) '\
                    'AND l.move_line_type = %s ' \
                    'AND l.company_id = %s '
                    'AND reconcile_id IS NULL '\
                    'AND '+query+' '\
                    'AND partner_id IS NOT NULL '\
                    'GROUP BY partner_id HAVING '+ having_where_clause,
                   'basic', [company_id] + having_values)
        res = cr.fetchall()
        if not res:
            return [('id','=','0')]
    ...
    def create(self, values):
        partner_code = False
        if 'is_company' not in values and 'parent_id' in values and values['parent_id']:
            values['customer'] = False
            values['supplier'] = False
            values['owner'] = False
            values['office_study'] = False
        else:
                ...
                if values['supplier']:
                    if values['provider_foreign_ok']:
                        ...
                        if 0 <= customer_indice < 9 :
                            code = ('E'+values['name2'][0]+'000'+str(customer_indice + 1)).upper()
                            values['customer_code'] = code
                        elif 9 <= customer_indice < 99 :
                            code = ('E'+values['name2'][0]+'00'+str(customer_indice + 1)).upper()
                            values['customer_code'] = code
                        elif 99 <= customer_indice < 999 :
                            code = ('E'+values['name2'][0]+'0'+str(customer_indice + 1)).upper()
                            values['customer_code'] = code
                        else :
                            code = ('E'+values['name2'][0]+str(customer_indice + 1)).upper()
                            values['customer_code'] = code
                        ...
                        property_account_expense = self.env['account.account'].search([('code','=','6111000600')])
                        values['property_account_expense'] = property_account_expense.id
                    elif values['local_ok']:
                        customer_indice = partner_code.supplier_indice_local
                        if 0 <= customer_indice < 9 :
                            code = ('L'+values['name2'][0]+'000'+str(customer_indice + 1)).upper()
                            values['customer_code'] = code
                        elif 9 <= customer_indice < 99 :
                            code = ('L'+values['name2'][0]+'00'+str(customer_indice + 1)).upper()
                            values['customer_code'] = code
                        elif 99 <= customer_indice < 999 :
                            code = ('E'+values['name2'][0]+'0'+str(customer_indice + 1)).upper()
                            values['customer_code'] = code
                        else :
                            code = ('L'+values['name2'][0]+str(customer_indice + 1)).upper()
                            values['customer_code'] = code
                        ...
                        account = self.env['account.account'].create(record_account)
                        values['property_account_payable'] = account.id
                        property_account_expense = self.env['account.account'].search([('code','=','6111000000')])
                        values['property_account_expense'] = property_account_expense.id
                    else :
                        customer_indice = partner_code.supplier_indice_several
                        if 0 <= customer_indice < 9 :
                            code = ('D'+values['name2'][0]+'000'+str(customer_indice + 1)).upper()
                            values['customer_code'] = code
                        elif 9 <= customer_indice < 99 :
                            code = ('D'+values['name2'][0]+'00'+str(customer_indice + 1)).upper()
                            values['customer_code'] = code
                        elif 99 <= customer_indice < 999 :
                            code = ('E'+values['name2'][0]+'0'+str(customer_indice + 1)).upper()
                            values['customer_code'] = code
                        else :
                            code = ('D'+values['name2'][0]+str(customer_indice + 1)).upper()
                            values['customer_code'] = code
    ...
    def action_done(self):
        ...
        record_history_followup = {
            ...
            'number': False,
            'deadline_date': False,
            'deadline_delay': False,
            'invoice_amount': False,
            'note_invoice': False
        }
        for invoice in self.unreconciled_aml_ids :
            record_history_followup['invoice_date'] = invoice.date
            record_history_followup['number'] = invoice.move_id.name
            record_history_followup['deadline_date'] = invoice.date_maturity
            record_history_followup['deadline_delay'] = invoice.deadline_delay
            record_history_followup['invoice_amount'] = invoice.result
            record_history_followup['note_invoice'] = invoice.note
            self.env['account.followup.history'].create(record_history_followup)
        self.write({'payment_next_action_date': False, 'payment_next_action': '', 'payment_responsible_id': False,'payment_note': False})
        return True
    ...
    def unpaid_action_done(self):
        ...
        for invoice in self.unpaid_aml_ids :
            ...
            record_history_followup['note_invoice'] = invoice.note
            self.env['account.followup.history'].create(record_history_followup)
        partner.write({'payment_next_action_date': False, 'payment_next_action': '', 'payment_responsible_id': False, 'unpaid_note': False})
        return True
...
class history_account_followup(models.Model):
    ...
    invoice_date = fields.Datetime('Date de Facture')
    ...
    note_invoice = fields.Text('Notes')
    type = fields.Selection([('followup', 'Relance'), ('impaid', 'Impayé')], 'Type')
