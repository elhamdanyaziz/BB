...
class stock_removal_cancel(models.TransientModel):
    ...
    type_action = fields.Selection([('partial_cancellation', 'Annulation Partielle'),('total_cancellation', 'Annulation Totale')], 'Opération', required=True, default='partial_cancellation')
    action1 = fields.Selection([('customer_request', 'Demande Client'),('internal_request', 'Demande Interne')], 'Qualification de l\'Annulation', required=False,default='customer_request')
    action2 = fields.Selection([('customer_request', 'Demande Client'),('internal_request', 'Demande Interne'),('product_not_available','Article non disponible en stock'),('defective_product','Article défectueux'),('product_to_repair','Article à réparer')], 'Qualification de l\'Annulation', required=False,default='customer_request')
    filter = fields.Selection([('draft', 'Brouillon'),('validated', 'Validée'),('approuved', 'Approuvé'),('ready_to_deliver', 'Prêt à livrer')],'Statut',required=False)
    ...
    def default_get(self, fields):
        ...
        res = super(stock_removal_cancel, self).default_get(fields)
        removal_id = self._context.get('active_id', [])
        removal = self.env['stock.removal'].browse(removal_id)
        res.update(reservation_method=removal.reservation_method)
        return res
    ...
    #
    #                 # line.move_id.write({'removal_id': False})
    ...
    #             removal.action_cancel()
    #             if self.action1 == 'customer_request' or self.action1 == 'internal_request' :
    #                 for line in self.line_ids :
    #                     record_move = {
    #                      'invoice_state': 'none',
    ...
    #
    #                     line.move_id.write({'removal_id': False})
    #                     record_move['product_uom_qty'] = line.product_qty
    #
    #                     if self.action1 == 'customer_request':
    #                         situation.write({'qty_canceled': situation.qty_canceled + line.product_qty})
    #                     else:
    ...
    #                     #     line_normal_reservation.unlink()
    #                     if line_shipping_reservation:
    #                         line_shipping_reservation.move_id.write({'move_dest_id': False})
    #                         line_shipping_reservation.move_dest_id.action_cancel()
    #                     if line_purchase_reservation:
    #                         line_purchase_reservation.move_id.write({'move_dest_id': False})
    #                         line_purchase_reservation.move_dest_id.action_cancel()
    #
    ...
    #             removal.action_cancel()
    #             if self.action2 == 'customer_request' or self.action2 == 'internal_request':
    #                 for line in self.line_ids:
    #                     record_move = {
    #                         'invoice_state': 'none',
    #                         'date_expected': datetime.today(),
    ...
    #                     if line_shipping_reservation:
    #                         line_shipping_reservation.move_id.write({'move_dest_id': False})
    #                         line_shipping_reservation.unlink()
    #                     if line_purchase_reservation:
    #                         line_purchase_reservation.move_id.write({'move_dest_id': False})
    #                         line_purchase_reservation.unlink()
    #
    #                     move = move_obj.create(record_move)
    #                     move.action_confirm()
    #                     move.action_assign()
    ...
    #             #         record_inventory_line['product_qty'] = line.product_qty
    #             #
    #             #         # if line_normal_reservation:
    #             #         #     line_normal_reservation.unlink()
    #             #         if line_shipping_reservation:
    ...
    #                     else:
    #                         line.move_id.sale_order_line_id.write({'qty_available': line.move_id.sale_order_line_id.qty_available - line.product_qty})
    #                 elif line.product_qty < line.move_id.product_uom_qty:
    #
    #
    #                     if self.action1 == 'customer_request':
    ...
    #                     line.move_id.action_cancel()
    #                     if self.action1 == 'customer_request':
    #                         situation.write({'qty_canceled': situation.qty_canceled + line.move_id.product_uom_qty})
    #                     else:
    #                         line.move_id.sale_order_line_id.write({'qty_available': line.move_id.sale_order_line_id.qty_available - line.move_id.product_uom_qty})
    ...
    #                             line.move_id.sale_order_line_id.write({'qty_available': line.move_id.sale_order_line_id.qty_available - line.product_qty})
    #                     elif line.product_qty < line.move_id.product_uom_qty:
    #                         record_move['product_uom_qty'] = line.move_id.product_uom_qty - line.product_qty
    #
    #                         if line_normal_reservation:
    #                             line_normal_reservation.write({'product_qty': line.product_qty})
    #                         if line_shipping_reservation:
    ...
    #
    #                         line.move_id.write({'removal_id':False})
    #                         record_move['product_uom_qty'] = line.move_id.product_uom_qty
    #                         # line.move_id.action_cancel()
    #
    #                         if line_normal_reservation:
    #                             line_normal_reservation.unlink()
    ...
    #
    #                         move_picking.action_cancel()
    #                         move_picking.write({'picking_id': False})
    #                         line.move_id.write({'removal_id':False})
    #                         record_move['product_uom_qty'] = line.product_qty
    #                         # line.move_id.action_cancel()
    #
    ...
    #                     elif line.product_qty < line.move_id.product_uom_qty:
    #                         line.move_id.sale_order_line_id.write({'qty_available': line.move_id.sale_order_line_id.qty_available - (line.move_id.product_uom_qty - line.product_qty)})
    #                         record_inventory_line['product_qty'] = line.move_id.product_uom_qty - line.product_qty
    #                         move_picking.write({'product_uom_qty': line.product_qty})
    #                         line.move_id.write({'product_uom_qty': line.product_qty})
    #
    #                         if line_normal_reservation:
    ...
    #                         line.move_id.write({'removal_id':False})
    #                         # line.move_id.action_cancel()
    #                         record_inventory_line['product_qty'] = line.move_id.product_uom_qty
    #
    #                         if line_normal_reservation:
    #                             line_normal_reservation.unlink()
    #                         if line_shipping_reservation:
    ...
    def create_removal_cancellation(self):
        ...
        removal = removal_obj.browse(removal_id)
        if self.type_action == 'total_cancellation' :
            if removal.reservation_method == 'stock':
                if removal.state == 'draft':
                    removal.action_cancel()
                    for line in self.line_ids:
                        line.move_id.action_cancel()
            ...
            if removal.reservation_method == 'shipping':
                shipping_reservation = self.env['stock.shipping.reservation'].search([('removal_id', '=', removal.id)])
                for line in self.line_ids:
                    situation = situation_obj.search([('sale_order_id', '=', line.removal_id.sale_order_id.id), ('product_id', '=', line.product_id.id)])
                    line_shipping_reservation = self.env['stock.shipping.reservation.line'].search([('removal_id', '=', removal.id), ('product_id', '=', line.product_id.id)])
                    line.move_id.write({'move_src_id': False, 'state': 'draft'})
                    line.move_id.action_cancel()
                    line_shipping_reservation.move_id.write({'move_dest_id': False})
                    line_shipping_reservation.write({'state': 'cancel'})
                    if self.action1 == 'customer_request':
                        situation.write({'qty_canceled': situation.qty_canceled + line.product_qty})
                    else:
                        line.move_id.sale_order_line_id.write({'qty_available': line.move_id.sale_order_line_id.qty_available - line.product_qty})
                removal.write({'state': 'cancel'})
                shipping_reservation.write({'state': 'cancel'})
            if removal.reservation_method == 'none':
                for line in self.line_ids:
                    situation = situation_obj.search([('sale_order_id', '=', line.removal_id.sale_order_id.id), ('product_id', '=', line.product_id.id)])
                    if self.action1 == 'customer_request':
                        situation.write({'qty_canceled': situation.qty_canceled + line.product_qty})
                        ...
                        line.move_id.sale_order_line_id.write({'qty_available': line.move_id.sale_order_line_id.qty_available - line.product_qty})
                picking = self.env['stock.picking'].search([('order_id', '=', removal.sale_order_id.id), ('removal_id', '=', removal.id)])
                picking.action_cancel()
                removal.write({'state': 'cancel'})
                ...
                if removal.state == 'draft':
                    ...
                    for line in self.line_ids:
                        ...
                        if line.product_qty == line.move_id.product_uom_qty:
                            ...
                            # line.move_id.write({'removal_id': False})
                            if line_normal_reservation:
                                line_normal_reservation.write({'state': 'cancel'})
                            if self.action1 == 'customer_request':
                                situation.write({'qty_canceled': situation.qty_canceled + line.product_qty})
                            else:
                                line.move_id.sale_order_line_id.write({'qty_available': line.move_id.sale_order_line_id.qty_available - line.product_qty})
