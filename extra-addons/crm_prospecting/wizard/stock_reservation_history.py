        ...
        cr.execute("""
            CREATE OR REPLACE VIEW stock_normal_reservation_history AS (
              SELECT
                ...
                GROUP BY snrl.id,sr.state,sr.name,snr.delivery_date, snr.partner_id,snr.location_id,snr.location_dest_id,snr.section_id,snr.user_id,snr.sale_order_id,snr.removal_id,snrl.product_id,snrl.product_qty,snrl.product_uom_id,snrl.move_id,snrl.sale_order_line_id
            )""")

class stock_removal_reservation(models.Model):
    _name = 'stock.removal.reservation'
    _auto = False
    _order = 'delivery_date asc'
    ...
    def init(self, cr):
        tools.drop_view_if_exists(cr, 'stock_removal_reservation')
        cr.execute("""
            CREATE OR REPLACE VIEW stock_removal_reservation AS (
              SELECT
                move.id as id,
                sr.state,
                sr.name,
                ...
                move.product_uom as product_uom_id,
                move.sale_order_line_id
                FROM
                    stock_removal sr,stock_move move
                WHERE sr.id=move.removal_id AND move.state = 'done' AND sr.state in ('approuved','ready_to_deliver')
                GROUP BY move.id,sr.state,sr.name,sr.delivery_date, sr.partner_id,sr.location_id,sr.location_dest_id,sr.section_id,sr.user_id,move.sale_order_id,move.removal_id,move.product_id,move.product_uom_qty,move.product_uom,move.id,move.sale_order_line_id
            )""")
